...
export const leagueController = {
  ...
  getAll: async (req, res, next) => {
    try {
      ...
      const leaguesWithJoinLink = leagues.map((league) => {
        ...
        const leagueName = leagueObj.league_name
          .replace(/\s+/g, "")
          .toLowerCase();
  ...
  update: async (req, res, next) => {
    try {
      ...
      if (league.created_by !== req.user.id) {
        return next(
          new AppError("You are not allowed to update this league", 403),
        );
      }
  ...
  delete: async (req, res, next) => {
    try {
      ...
      if (league.created_by !== req.user.id) {
        return next(
          new AppError("You are not allowed to delete this league", 403),
        );
      }
