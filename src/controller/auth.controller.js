...
const signup = catchAsync(async (req, res, next) => {
  ...
  return res.status(201).json({
    status: "success",
    message:
      "Please check your email for verification code. Code expires in 15 minutes.",
...
const refresh = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const payload = {
      ...
      isAdmin:
        decoded.user_type === ROLES.SUPER_ADMIN ||
...
const playerAuthentication = catchAsync(async (req, _, next) => {
  let idToken = "";
  const requestedUrl = req.originalUrl;
  if (!requestedUrl.includes("register")) {
    return next();
  }
  if (
    req.headers.authorization &&
  ...
  try {
    ...
    // 3. get the user detail from db and add to req object
    const freshUser = await PlayerDetail.findByPk(tokenDetail.id, {
      raw: true,
      attributes: {
        exclude: [
          "otp",
    ...
    if (!freshUser) {
      return next(new AppError("Player no longer exists", 401));
    }
