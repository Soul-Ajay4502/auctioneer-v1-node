...
function detectVisualizationRequest(query) {
  ...
  if (
    ...
    (queryLower.includes("breakdown") &&
      (queryLower.includes("by type") || queryLower.includes("by status")))
  ) {
    return "pie";
  }
  ...
  switch (visualizationType) {
    ...
    case "timeline": {
      ...
      const tasksWithDates = results
        .filter(
          (result) =>
            result.payload.type === "task" &&
            result.payload.start_date &&
            result.payload.due_date,
        )
        .map((result) => ({
